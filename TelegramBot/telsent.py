...
def handle_docs_photo(message):
    try:
        ...
        inf=Info(shopname="none",datatime=datetime.datetime.now() , user_id=message.chat.id,
                 status=0,photo_name=photo_name)#Status - состояние полки по мнению нейросети
        inf.save()
...
def repeat_all_messages(message): # Название функции не играет никакой роли, в принципе
    ...
    if len( Users.select().where(Users.tel_id==message.chat.id) )==0:
        u = Users( tel_id=message.chat.id, name=message.from_user.last_name,
...
def repeat_all_messages(message):
    if(message.text=="Get Balanse"):
        ...
        bot.send_message(message.chat.id, str(b.balance)+" баллов")
